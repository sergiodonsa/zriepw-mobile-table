sap.ui.define([
	...
	"use strict";
	var TableController = Controller.extend("zriepwmobiletable.controller.Table", {
		_oEventBus: null,
		_oFilterDialog : null,
		_oSortDialog : null,
		_oTable : null,
...
//				aFilters = [],
//				sPath = 'ANLAGE',//Filtro i valori sporchi della tabella originale per chiave = '_________'
//				sOperator = 'NE',
...
//FINE CHOOSE         
		...
		onSearch: function (oEvent) {
			...
			if (oEvent.getParameters().clearButtonPressed) {
				...
				if (sQuery && sQuery.length > 0) {
					oBindings.filter([new Filter("P_IOUTTAB-ANLAGE", FilterOperator.Contains, sQuery)]);
				}
			}
		},
		...
		onSort: function(){
			if (!this._oSortDialog) {
				this._oSortDialog = sap.ui.xmlfragment("zriepwmobiletable.fragments.Sort", this);
			}
		...
		/*onFilter: function(){
			if (!this._oFilterDialog) {
				this._oFilterDialog = sap.ui.xmlfragment("zriepwmobiletable.fragments.Filter", this);
			}
