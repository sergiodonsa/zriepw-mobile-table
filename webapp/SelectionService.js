sap.ui.define([
	...
	"use strict";
	var oSelectionService = Service.extend("zriepwmobiletable.SelectionService", {
		
		_oEventProvider : null,
		
		init: function () {
			ServiceFactoryRegistry.register("zriepwmobiletable.SelectionService", new ServiceFactory(this.getInterface()));
			this._oEventProvider = new sap.ui.base.EventProvider();
			//The "select" event is published by the zriepwmobiletable.controller.Table controller instance once a table row has been selected
			sap.ui.getCore().getEventBus().subscribe("zriepwmobiletable", "select", this._onSelect, this);
		},
		/*
		 * @private
		 ...
		 * @param {String} sId
		 * @param {Object} oData
		 * The method propagates the "select" event fired by the zriepwmobiletable.controller.Table instance
		 */
